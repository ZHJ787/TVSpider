...
class jableSpider extends NodeJSSpider {
    constructor() {
        super();
    }
    ...
    async init(inReq, _outResp) {
        ...
        if (this.getAppName().indexOf("阿里") > -1 || this.getAppName().indexOf("厂长") > -1 || this.getAppName() === "push_agent") {
            await spider.initCloud(inReq.server.config)
    ...
    async setSearch(wd, quick,pg) {
        await spider.setSearch(wd, quick,pg)
        this.vodList = spider.vodList
        this.result = spider.result
